...
{
  const char *country;
  long gmtOffset_sec;     // UTC offset in seconds (e.g., India: 5*3600 + 30*60 = 19800)
  int daylightOffset_sec; // Usually 0 or 3600 if DST applies (most countries in table have no DST)
};

const CountryOffset offsetTable[] = {
    {"india", 19800, 0},      // +5:30
    {"pakistan", 18000, 0},   // +5:00
    {"bangladesh", 21600, 0}, // +6:00
    {"sri_lanka", 19800, 0},  // +5:30
    {"nepal", 20700, 0},      // +5:45
    {"uk", 0, 3600},          // GMT/BST (has DST)
    {"germany", 3600, 3600},  // CET/CEST
    {"france", 3600, 3600},
    {"spain", 3600, 3600},
    {"italy", 3600, 3600},
    {"netherlands", 3600, 3600},
    {"sweden", 3600, 3600},
    {"norway", 3600, 3600},
    {"finland", 7200, 7200}, // EET/EEST (usually +2/+3)
    {"denmark", 3600, 3600},
    {"switzerland", 3600, 3600},
    {"austria", 3600, 3600},
    {"belgium", 3600, 3600},
    {"ireland", 0, 3600},  // GMT/IST
    {"portugal", 0, 3600}, // WET/WEST (mainland)
    {"poland", 3600, 3600},
    {"czech_republic", 3600, 3600},
    {"hungary", 3600, 3600},
    {"greece", 7200, 7200},
    {"turkey", 10800, 0}, // +3 no DST
    {"egypt", 7200, 0},
    {"uae", 14400, 0},
    {"argentina", -10800, 0},
    {"mexico", -21600, 3600}, // Varies, but common
    {"usa", -18000, 3600},    // Eastern
    {"japan", 32400, 0},
    {"china", 28800, 0},
    {"south_korea", 32400, 0},
    {"singapore", 28800, 0},
    {"australia", 36000, 3600}, // Sydney time (AEDT/AEST)
    {"brazil", -10800, 0},      // Brasília
    {"canada", -18000, 3600},   // Eastern
    {"russia", 10800, 0},       // Moscow
    {"south_africa", 7200, 0},
    {"usa-west", -28800, 3600},
    {"usa-east", -18000, 3600},
    {"usa-central", -21600, 3600},
    {"usa-mountain", -25200, 3600},
    {"canada-east", -18000, 3600},
    {"canada-pacific", -28800, 3600},
    {"canada-atlantic", -14400, 3600},
    {"utc", 0, 0}};

const int OFFSET_COUNT = sizeof(offsetTable) / sizeof(offsetTable[0]);
const int TZ_COUNT = sizeof(tzTable) / sizeof(tzTable[0]);
...
void Spotify::begin()
{
  SPOTIFY_LOGI(_TAG, "Initializing Clock...");
  if (!setupTimeInteractive())
  {
    SPOTIFY_LOGE(_TAG, "Failed to initialize time");
  }
  ...
  if (!is_auth())
  {
    ...
    char url[1024];
    snprintf(url, sizeof(url), "https://accounts.spotify.com/authorize"
                               "?client_id=%s"
                               "&response_type=code"
                               "&redirect_uri=%s"
                               "&scope=%s"
                               "&state=%s",
             _client_id, _redirect_uri, scopes, _random_state);
...
/**
 * Parse detailed currently playing information with full multiple artist support
 */
bool Spotify::parseCurrentTrack(JsonDocument& doc, SpotifyCurrentTrack& out)
{
  ...
  out.timestamp = doc["timestamp"].as<unsigned long long>();

  if (out.timestamp > 0) {
    out.fetchTimeLocal = epochToDateTime(out.timestamp, "%Y-%m-%d %H:%M:%S");
  }

  JsonObject item = doc["item"];

  out.trackName = item["name"] | "";
  out.trackId = item["id"] | "";
  out.trackUri = item["uri"] | "";
  out.durationMs = item["duration_ms"] | 0;
  out.explicitTrack = item["explicit"] | false;
  out.popularity = item["popularity"] | 0;

  // === Track direct open link ===
  if (item.containsKey("external_urls") && !item["external_urls"]["spotify"].isNull()) {
    out.trackLink = item["external_urls"]["spotify"].as<String>();
  } else {
    out.trackLink = "https://open.spotify.com/track/" + out.trackId;
  }
  ...
  for (JsonObject artist : artists) {
    if (out.artistCount >= out.MAX_ARTISTS) break;

    out.artistNames[out.artistCount] = artist["name"] | "";
    out.artistIds[out.artistCount] = artist["id"] | "";
    out.artistUris[out.artistCount] = artist["uri"] | "";

    // Artist direct link
    if (artist.containsKey("external_urls") && !artist["external_urls"]["spotify"].isNull()) {
      out.artistLinks[out.artistCount] = artist["external_urls"]["spotify"].as<String>();
    } else {
      out.artistLinks[out.artistCount] = "https://open.spotify.com/artist/" + out.artistIds[out.artistCount];
    }
  ...
  out.albumName = album["name"] | "";
  out.albumId = album["id"] | "";
  out.albumUri = album["uri"] | "";
  out.releaseDate = album["release_date"] | "";
  out.albumType = album["album_type"] | "";

  // Album direct link
  if (album.containsKey("external_urls") && !album["external_urls"]["spotify"].isNull()) {
    out.albumLink = album["external_urls"]["spotify"].as<String>();
  } else {
    out.albumLink = "https://open.spotify.com/album/" + out.albumId;
  }

  // === Album Cover Images (direct URLs) ===
  JsonArray images = album["images"];
  if (images.size() > 0) {
    out.albumImageUrlLarge = images[0]["url"].as<String>();  // Usually 640x640
    if (images.size() > 1) out.albumImageUrlMedium = images[1]["url"].as<String>();
    if (images.size() > 2) out.albumImageUrlSmall = images[2]["url"].as<String>();
  }

  // === Context (playlist, album, liked songs, etc.) ===
  if (doc.containsKey("context") && !doc["context"].isNull()) {
    JsonObject context = doc["context"];
    out.contextType = context["type"] | "";
    out.contextUri = context["uri"] | "";

    if (context.containsKey("external_urls") && !context["external_urls"]["spotify"].isNull()) {
      out.contextLink = context["external_urls"]["spotify"].as<String>();
    } else if (out.contextType == "collection") {
      out.contextLink = "https://open.spotify.com/collection/tracks";
    }
  }

  // Actions
  if (doc.containsKey("actions") && doc["actions"].containsKey("disallows")) {
    out.resumingDisallowed = doc["actions"]["disallows"]["resuming"] | false;
  }

  return true;
}

String Spotify::askUserCountry()
{
  Serial.println();
  Serial.println("🕒 Timezone setup");
  Serial.println("Enter your country (example: india, usa, germany, utc):");
  Serial.print("> ");
...
const char *Spotify::findTimezoneForCountry(const String &country)
{
  for (int i = 0; i < TZ_COUNT; i++)
  {
    if (country == tzTable[i].country)
    {
      return tzTable[i].tz;
    }
  }
...
bool Spotify::setupTimeInteractive()
{
  String country = askUserCountry();
  const char *tz = findTimezoneForCountry(country); // Keep for nice logging
  ...
  {
    if (country.equalsIgnoreCase(offsetTable[i].country))
    {
      gmtOffset = offsetTable[i].gmtOffset_sec;
      daylightOffset = offsetTable[i].daylightOffset_sec;
      found = true;
      break;
    }
  }
  ...
  if (!found)
  {
    Serial.println("❌ Unknown country - falling back to UTC");
    gmtOffset = 0;
    daylightOffset = 0;
    tz = "UTC";
  }
  ...
  // Sync NTP with explicit offsets (this is the reliable way on ESP32)
  configTime(gmtOffset, daylightOffset, "pool.ntp.org", "time.nist.gov", "time.google.com");
...
/**
 * Convert millisecond epoch timestamp to local formatted time string
 * @param timestamp_ms Epoch timestamp in milliseconds (e.g., from Spotify API)
 * @param format Optional format string (default: "YYYY-MM-DD HH:MM:SS")
 *               See: https://www.cplusplus.com/reference/ctime/strftime/
 * @return String with formatted local time
 */
String Spotify::epochToDateTime(unsigned long long timestamp_ms, const char *format)
{
  ...
  timeinfo = *gmtime(&timestamp_sec); // First get UTC
  // Then convert to local time using current timezone settings
...
String Spotify::getCurrentTimeString()
{
  struct tm timeinfo;
  if (!getLocalTime(&timeinfo))
  {
...
String Spotify::getArtistNamesCombined(const SpotifyCurrentTrack& track)
{
  String combined = "";
  for (int i = 0; i < track.artistCount; i++) {
    if (i > 0) combined += ", ";
    combined += track.artistNames[i];
  }
  return combined;
}